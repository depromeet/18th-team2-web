...
import { usePartyEnded } from '@/services/party-ended';

export default function PartyEndedPage() {
  const { partyId } = useParams<{ partyId: string }>();
  const navigate = useNavigate();
  const { data } = usePartyEnded(partyId ?? '');

  if (!data) return null;

  const writableFrom = new Date(data.writableFrom);
  const writableUntil = new Date(data.writableUntil);
          ...
          {isExpired ? (
            ...
            <div className="flex w-full flex-col items-center gap-1">
              <H3 as="p" className="text-center font-medium text-black">
                {data.hostName}님의 파티가 <span className="text-red-500">종료</span>되었어요
              </H3>
