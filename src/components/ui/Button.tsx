...
export function Button({
  ...
  return (
    <button
      className={`inline-flex cursor-pointer items-center justify-center gap-2 text-body-1 font-semibold disabled:cursor-not-allowed ${sizeStyles[size]} ${variantStyles[variant]} ${className ?? ''}`}
      {...props}
    >
      {leftIcon}
      {children}
